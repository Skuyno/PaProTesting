...
`candidate-service` по умолчанию запускает 4 воркер-процесса uvicorn за одним
портом 8080 (`WEB_CONCURRENCY`, см. «Конфигурация») — конкурентность и
восстановление работают между настоящими процессами ОС, не только внутри
одного event loop.
...
|-------------------------------|-----------------------|-------------------------------|
| WEB_CONCURRENCY               | 4                     | число воркер-процессов uvicorn за портом 8080 |
...
Мультиинстансность заданием не требовалась, но захотелось разобраться,
как это устроено на практике — так candidate-service и обзавёлся
координацией через Postgres.
